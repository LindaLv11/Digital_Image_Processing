...
int main(){
    	...
    	for(int i =1;i < (height-1);++i)
	{
		...
		{
		    if ((*(imagedata + i * width + j)).blue>200)
            {
                seed[i][j]=255;
		...
		for(int j = 1;j <  (width-1);++j)
		{
		    if (seed[i][j]==255)
            {
                for(n=i-1; n<=i+1;n++)
                {
                   ...
                   {
                     if(seed[n][m]==0&&fabs((*(imagedata + n * width + m)).blue-(*(imagedata + i * width + j)).blue)<5&&n!=i&&m!=j)
                   {
                      seed[n][m]=255;
                      count1=1;
                   }
 ...
 for(int i =1;i < (height-1);++i)
	{
		for(int j = 1;j <  (width-1);++j)
		{
		    (*(imagedataRot + i * width + j)).blue=seed[i][j];
		}
	}
